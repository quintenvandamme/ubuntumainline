...
while [[ $# -gt 0 ]]; do
  ...
  case "${1}" in
    ...
    -arm|--arm64)
      mkdir /tmp/ubuntukernel$KERNEL_VER
      cd /tmp/ubuntukernel$KERNEL_VER
      wget https://kernel.ubuntu.com/~kernel-ppa/mainline/v5.10.63/arm64/linux-headers-5.10.63-051063-generic_5.10.63-051063.202109080732_arm64.deb
      wget https://kernel.ubuntu.com/~kernel-ppa/mainline/v5.10.63/arm64/linux-image-unsigned-5.10.63-051063-generic_5.10.63-051063.202109080732_arm64.deb
      wget https://kernel.ubuntu.com/~kernel-ppa/mainline/v5.10.63/amd64/linux-modules-5.10.63-051063-generic_5.10.63-051063.202109080732_amd64.deb
      sudo dpkg -i *.deb
      cd
      rm -r /tmp/ubuntukernel$KERNEL_VER
